        ...
        (new Traverser {
          override def traverse(tree: Tree) = {
            if (tree.pos != NoPosition) {
              tokens.find(_.start == tree.pos.point - headerOffset).foreach {
                token =>
                  tree match {
                    case tree: TypeTree =>
          ...
          symbol match {
            ...
            case t: TermSymbol =>
              findPackage(t.typeSignature.typeSymbol.fullName).flatMap {
                case (_, link) =>
                  scalaDocLink(t.typeSignature.typeSymbol).map {
                    encodedTypeName =>
                      TypeAnnotation(
      ...
      def generate(
        ...
        def formatComments(
          ...
          val transformer = Transformer
            ...
            .to(HTML)
            .rendering {
              case (fmt, SpanLink(content, url, _, opt))
                  if findPackage(url.render()).isDefined =>
                fmt.element(
                  "a",
                  opt,
                  content,
